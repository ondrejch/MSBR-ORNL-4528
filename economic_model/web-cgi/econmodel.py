...
if len(cgi_fields.keys()) == 0 :  # default inputs
    ...
    grid_size   = cgi_fields.getvalue('grid_size')
    storage_eff = cgi_fields.getvalue('storage_eff')
    heat_eff    = cgi_fields.getvalue('heat_eff')
    solar_fract = cgi_fields.getvalue('solar_fract')
...
</form>''' . format(grid_size, storage_eff, heat_eff, solar_fract)
...
if len(cgi_fields.keys()) == 4:
    size_data = plant_size(1e9*grid_size, storage_eff, heat_eff, solar_fract, do_plot, dir_figs)
    print( size_data )
