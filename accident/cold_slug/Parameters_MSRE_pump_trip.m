...
% Cold Slug
colddata = [0 482.22 0];
coldtime = [0 1000 1007.48]; % 20 ft^3
%coldtime = [0 1000 1011.22]; % 30 ft^3
...
tau_l  = 16.73/flow_frac; % ORNL-TM-0728 %16.44; % (s)
tau_c  = 8.46/flow_frac; % ORNL-TM-0728 %8.460; % (s)
...
%a_f    = -6.13E-5*(9/5); % U233 
a_f    = -8.460E-05; % U235 (drho/°C) fuel salt temperature-reactivity feedback coefficient ORNL-TM-1647 p.3 % -5.904E-05; % ORNL-TM-0728 p. 101 %
%a_g    = -3.23E-5*(9/5); % U233 
a_g    = -4.680E-05; % U235 (drho/°C) graphite temperature-reactivity feedback coefficient ORNL-TM-1647 p.3 % -6.624E-05; % ORNL-TM-0728 p.101
...
W_f      = vdot_f*rho_f*flow_frac; % 182.78; % calcd from m_dot*cp*delT=P; vdot_f*rho_f; % fuel flow rate (kg/s)
...
tau_hx_c = 8.67/flow_frac; % (sec) delay from hx to core TDAMSRE p.6
tau_c_hx = 3.77/flow_frac; % (sec) subtracted 1 sec for external loop power generation node resident time; delay from core to fuel hx TDAMSRE p.6
