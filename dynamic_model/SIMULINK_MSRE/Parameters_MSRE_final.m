...
tau_l  = 16.73; % ORNL-TM-0728 %16.44; % (s) loop transit time (polimi thesis)
tau_c  = 8.46; % ORNL-TM-0728 %8.460; % (s) core transit time (polimi thesis)
P      = 10; % Thermal Power in MW ORNL-TM-1070, p.2
...
W_f      = vdot_f*rho_f; % 182.78; % calcd from m_dot*cp*delT=0.86*P; vdot_f*rho_f; % fuel flow rate (kg/s)
...
scp_f    = cp_f/m_f;% specific heat capacity of fuel salt (MJ/kg-C) ORNL-TM-0728 p.8
...
cp_g     = 3.58*9/5; % TDAMSRE p.5 graphite total heat capacity (MW-s/C) ORNL-TM-1647 p.3
...
k_g      = 0.066; % fraction of total power generated in the graphite  ORNL-TM-0728 p.9
...
k_f      = 0.934;     % fraction of heat generated in fuel - that generated in external loop ORNL-TM-0728 p.9
...
cp_p = scp_f; % fuel heat capacity (MW-s/(kg-C))
...
W_s      = 332.79; % 108.33; % calcd from mdot*cp*delT; vdot_s*rho_s; % coolant flow rate (kg/s) ORNL-TM-1647 p.3 

m_s      = v_cool*in_m*rho_s; % coolant mass in PHE (kg) 
nn_s     = 4; % number of coolant nodes in PHE
mn_s     = m_s/nn_s; % coolant mass per node (kg)
cp_s     = 0.4*9/5; % (MJ/C) ORNL-TM-1647 p.9
scp_s    = 2.219E-03; % specific heat capacity of coolant (MJ/(kg-C) ORNL-TM-0728 p.8
...
scp_t    = 5.77E-04; % specific heat capacity of tubes (MJ/(kg-C)) ORNL-TM-0728 p.20      
...
% SECONDARY FLOW PARAMETERS - DONE
vdot_rs    = 100;%94.389; % ORNL-TM-0728 p. 296; 78.82; % air volume flow rate (m^3/s) ORNL-TM-1647 p.2
