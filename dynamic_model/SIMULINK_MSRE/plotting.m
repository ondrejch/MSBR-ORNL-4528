...
figure;plot(tout,msre_core_mux(:,[19 21]),[0,max(tout)],[T0_f1,T0_f1],[0,max(tout)],[T0_f2,T0_f2]); grid on;
figure;plot(tout,msre_core_mux(:,23),[0,max(tout)],[T0_out,T0_out]); grid on;
...
figure;plot(tout,msre_he_mux(:,[3 4 6 7])); grid on; legend('p1','p2','p3','p4');
figure;plot(tout,msre_he_mux(:,[8 9])); grid on; legend('t1','t2');
figure;plot(tout,msre_he_mux(:,[10 11 12 13])); grid on; legend('s1','s2','s3','s4');
